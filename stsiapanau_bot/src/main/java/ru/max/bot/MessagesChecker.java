...
import static ru.max.bot.tools.BotHelper.monthForEdit;
...
public class MessagesChecker implements Runnable {
                        ...
                        if ((typeCommand.equalsIgnoreCase("bot_command") || isMapper)) {
                            ...
                            switch (text) {
                                        ...
                                        try {
                                            ...
                                            if (document.isPresent()) {
                                                ...
                                                try {
                                                    ...
                                                    sb.append(DataBaseHelper.getInstance().getStatisticByMonth(rentHolder, isRus)).append("\n\n\n");
                                                    Double totalBefore = rentHolder.getTotalAmount();

                                                    setDefaultRentButtons(rh, isRus);
                                                    RatesHolder rates = DataBaseHelper.getInstance().getRatesForRecalc(id, objectMapper);
                                                    if (null != rates) {
                                                        rates.getLight().getRates().entrySet().forEach(e -> {
                                                            Counter c = rentHolder.getLight().get(e.getKey()).recalcCounter(e.getValue());
                                                            rentHolder.getLight().put(e.getKey(), c);
                                                        });

                                                        rentHolder.getColdWater().entrySet().forEach(e -> {
                                                            Counter c = e.getValue().recalcCounter(rates.getWater().getColdWaterRate());
                                                            rentHolder.getColdWater().put(e.getKey(), c);
                                                        });

                                                        rentHolder.getHotWater().entrySet().forEach(e -> {
                                                            Counter c = e.getValue().recalcCounter(rates.getWater().getHotWaterRate());
                                                            rentHolder.getHotWater().put(e.getKey(), c);
                                                        });

                                                        Counter c = rentHolder.getOutfall().recalcCounter(rates.getWater().getOutfallRate());
                                                        rentHolder.setOutfall(c);

                                                        sb.append("======================").append(DataBaseHelper.getInstance().getStatisticByMonth(rentHolder, isRus)).append("\n\n");

                                                        Double totalAfter = rentHolder.getTotalAmount();

                                                        sb.append((isRus) ? "<b>Разница:</b> " : "<b>Difference:</b> ").append(String.format("%.2f", (totalBefore - totalAfter))).append((isRus) ? " руб" : " rub");
                                ...
                                case "/setprimarycounters": {
                                    ...
                                    answer = (isRus) ? new StringBuilder()
                                            .append("На данных показаниях будут строиться дальнейшие расчеты аренды.\nВы можете использовать следующие команды:\n\n")
                                            ...
                                            .toString()
                                            : new StringBuilder()
                                            .append("In these indications will be built further lease payments.\nYou can set next primary counters via follows commands:\n\n")
                            ...
                            case 3:
                                try {
                                    lightObj.getIndications().put(lightObj.getPeriod(),
                                            Double.valueOf(text));
                                    answer = (isRus) ? new StringBuilder(
                                            "Показание для периода ")
                                            .append(lightObj.getPeriod())
                                            .append(" заданы успешно").toString()
                                            : new StringBuilder("Start indication for ")
                                            .append(lightObj.getPeriod())
    ...
    private String getRentMenu(String id, boolean isRus, ResponseHolder rh) {
        ...
        if (DataBaseHelper.getInstance().existRentUser(id)) {

            if (DataBaseHelper.getInstance().existPayment(id)) {
                if (isRus) {
            ...
            if (isRus) {
                ...
                answer = new StringBuilder()
                        .append("For access to all functions for control your rent you must set primary counters. Please use this command:\n")
